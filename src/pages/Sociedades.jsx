...
import { listSociedades, createSociedade, updateSociedade, deleteSociedade } from '../lib/db.js';
...
const FORMA_OBRIGAR_OPTS = [
  { v: 'qualquer_gerente', l: 'A sociedade obriga-se com a intervenção de qualquer gerente, isoladamente' },
  { v: 'dois_gerentes', l: 'A sociedade obriga-se com a intervenção de quaisquer dois gerentes, em conjunto' },
  { v: 'todos_gerentes', l: 'A sociedade obriga-se com a intervenção de todos os gerentes, em conjunto' },
  { v: 'gerente_unico', l: 'Gerente único — obriga com a sua intervenção isolada' },
  { v: 'qualquer_e_conjunto', l: 'Qualquer gerente isoladamente para atos de valor reduzido; dois em conjunto para demais' },
  { v: 'outro', l: 'Outra (especificar)' },
];
...
function errNIF(nif) {
  if (!nif) return null;
  return /^\d{9}$/.test(nif) ? null : 'NIF: exatamente 9 dígitos';
}
function errCC(num) {
  if (!num) return null;
  return /^\d{8} \d [A-Z]{2}\d$/.test(num) ? null : 'Formato: 12345678 0 ZY9';
}
function ccExpirado(val) {
  if (!val) return false;
...
export default function Sociedades({ socIdToOpen }) {
  ...
  const novo = () => setEditing({
    ...
    num_gerentes: 1, forma_obrigar: 'qualquer_gerente', forma_obrigar_outro: '',
    socios: [{ letra: 'A', quota: 0, pct: 100, tipo_pessoa: 'singular', nacionalidade: 'Portuguesa' }],
    _new: true,
  });
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input className="search-input" placeholder="Procurar firma ou NIPC..." value={busca} onChange={e => setBusca(e.target.value)} />
          ...
          {filtered.map(s => (
            <div key={s.id} className="card card-hover">
              ...
              <div style={{ fontSize: 12, color: '#6B7280', padding: '8px 0', borderTop: '1px solid #F3F4F6', borderBottom: '1px solid #F3F4F6' }}>
                ...
                <div style={{ marginTop: 2 }}>{s.sede}</div>
              </div>
...
function EditModal({ sociedade, onClose, onSave }) {
  ...
  const addSoc = () => setS(p => ({
    ...p,
    socios: [...p.socios, { letra: String.fromCharCode(65 + p.socios.length), quota: 0, pct: 0, tipo_pessoa: 'singular', nacionalidade: 'Portuguesa' }],
  }));
  ...
  const validar = () => {
    ...
    s.socios.forEach((soc, i) => {
      if (soc.tipo_pessoa !== 'coletiva') {
        const nifE = errNIF(soc.nif);
        if (nifE) e[`soc_${i}_nif`] = nifE;
        if (soc.doc_tipo === 'CC' || !soc.doc_tipo) {
          const ccE = errCC(soc.doc_num);
          if (ccE && soc.doc_num) e[`soc_${i}_cc`] = ccE;
          if (!ccE && soc.doc_num && ccExpirado(soc.doc_validade)) e[`soc_${i}_exp`] = 'Documento expirado';
        }
      }
    });
    setErros(e);
  ...
  const guardar = async () => {
    if (!validar()) { setTab('socios'); return; }
    await onSave({ ...s, capital: Number(s.capital) || 0 });
  };

  return (
    <Modal title={s._new ? 'Nova sociedade' : s.firma} size="lg" onClose={onClose} footer={
      <><Btn variant="secondary" onClick={onClose}>Cancelar</Btn><Btn onClick={guardar}>Guardar</Btn></>
    }>
      ...
      {tab === 'dados' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <Select label="Tipo de sociedade" value={s.tipo} onChange={v => upd('tipo', v)} options={TIPOS} />
          <div style={{ gridColumn: '1/-1' }}><Input label="Firma" value={s.firma} onChange={v => upd('firma', v)} required /></div>
          <div style={{ gridColumn: '1/-1' }}><Input label="Sede (morada completa)" value={s.sede} onChange={v => upd('sede', v)} required /></div>
          ...
          <div style={{ gridColumn: '1/-1' }}>
            <Select label="Forma de obrigar" value={s.forma_obrigar} onChange={v => upd('forma_obrigar', v)} options={FORMA_OBRIGAR_OPTS} />
            {s.forma_obrigar === 'outro' && (
              <div style={{ marginTop: 8 }}>
                <Input label="Especifique a forma de obrigar" value={s.forma_obrigar_outro} onChange={v => upd('forma_obrigar_outro', v)} placeholder="Texto completo para o pacto social..." />
              </div>
            )}
          </div>
      ...
      {tab === 'socios' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
            <p style={{ fontSize: 12, color: '#6B7280' }}>Identificação completa de cada sócio.</p>
              ...
              <div key={i} style={{ background: '#FAFAF8', padding: 16, borderRadius: 8, marginBottom: 12, border: '1px solid #E5E7EB' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    <label className="field-label">Outras</label>
                  ...
                  {isColetiva ? (
                    <>
                      <div style={{ gridColumn: '1/-1' }}><Input label="Firma da sociedade sócia" value={soc.firma_socio} onChange={v => updSoc(i, 'firma_socio', v)} /></div>
                    ...
                    <>
                      ...
                      <div>
                        ...
                        {erros[`soc_${i}_nif`] && <div style={{ fontSize: 11, color: '#B91C1C', marginTop: 2 }}>⚠ {erros[`soc_${i}_nif`]}</div>}
                      </div>
                      <Input label="Nacionalidade" value={soc.nacionalidade || 'Portuguesa'} onChange={v => updSoc(i, 'nacionalidade', v)} />
                      <Select label="Tipo doc." value={soc.doc_tipo || 'CC'} onChange={v => updSoc(i, 'doc_tipo', v)} options={DOC_TIPOS} />
                      <div style={{ gridColumn: 'span 2' }}>
                        <Input label="N.º documento" value={soc.doc_num} onChange={v => updSoc(i, 'doc_num', v)} placeholder={soc.doc_tipo !== 'PASSAPORTE' ? '12345678 0 ZY9' : ''} />
                        {(soc.doc_tipo === 'CC' || !soc.doc_tipo) && erros[`soc_${i}_cc`] && <div style={{ fontSize: 11, color: '#B91C1C', marginTop: 2 }}>⚠ {erros[`soc_${i}_cc`]}</div>}
                      </div>
                      <div>
                        <Input label="Validade" value={soc.doc_validade} onChange={v => updSoc(i, 'doc_validade', v)} placeholder="DD/MM/AAAA" />
                        {(erros[`soc_${i}_exp`] || (soc.doc_validade && ccExpirado(soc.doc_validade))) && (
                          <div style={{ fontSize: 11, color: '#D97706', marginTop: 2 }}>⚠ Documento expirado</div>
                        )}
                      </div>
                      ...
                      <Input label="Natural de (freguesia)" value={soc.natural_freguesia} onChange={v => updSoc(i, 'natural_freguesia', v)} />

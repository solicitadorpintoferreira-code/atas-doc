...
export async function generateAtaSimples(soc, opts = {}) {
  ...
  if (deliberacoes.length > 0) {
    deliberacoes.forEach((d, i) => {
      c.push(p(`Ponto ${PONTOS_EXTENSO[i] || i + 1} — ${d}`, { before: 300, bold: true }));
      c.push(p('[CONTEÚDO DA DELIBERAÇÃO — preencher manualmente]', { before: 100 }));
      c.push(p('A deliberação foi aprovada por unanimidade dos votos correspondentes à totalidade do capital social.', { before: 80 }));
    });
  } else {
    c.push(p('Ponto Único:', { before: 300, bold: true }));
    c.push(p('[CONTEÚDO DA DELIBERAÇÃO — preencher manualmente]', { before: 100 }));
    c.push(p('A deliberação foi aprovada por unanimidade dos votos correspondentes à totalidade do capital social.', { before: 80 }));
  }

...
function idSocio(s) {
  ...
  if (s.tipo_pessoa === 'coletiva') {
    let r = `${s.firma_socio || '[FIRMA]'}, pessoa coletiva com o NIPC ${s.nipc_socio || '[NIPC]'}, com sede em ${s.sede_socio || '[SEDE]'}`;
    if (s.representante_nome) r += `, representada por ${s.representante_nome}${s.representante_cargo ? `, na qualidade de ${s.representante_cargo}` : ''}`;
    return r;
  }
  // Pessoa singular
  ...
  if (s.estado_civil) r += `, ${s.estado_civil}`;
  ...
  if (s.doc_num) r += `, portador(a) do ${s.doc_tipo || 'CC'} n.º ${s.doc_num}${s.doc_validade ? `, válido até ${s.doc_validade}` : ''}`;
...
export async function generateAtaSimples(soc, opts = {}) {
  ...
  c.push(p(`Aos ${data}, pelas ${hora} horas, reuniu, na sua sede social, sita em ${soc.sede || '[SEDE]'}, a Assembleia Geral ${uni ? 'do sócio único' : 'Extraordinária'} da sociedade ${soc.tipo?.toLowerCase() || 'por quotas'}, denominada "${soc.firma}", com o número de pessoa coletiva ${soc.nipc}, matriculada na Conservatória do Registo Comercial sob o mesmo número, com o capital social de ${fmtEur(soc.capital)}.`));
  ...
  (soc.socios || []).forEach((s, i) => {
    ...
    c.push(p(`${String.fromCharCode(97 + i)}) ${idSocio(s)}, titular de uma quota de ${fmtEur(s.quota)}, correspondente a ${s.pct}% do capital social${libStr}.`, { indent: true }));
  });
  ...
  if (deliberacoes.length > 0) {
    deliberacoes.forEach((d, i) => c.push(p(`Ponto ${['Um', 'Dois', 'Três', 'Quatro', 'Cinco'][i] || (i + 1)}: ${d}`, { bold: true, before: 200 })));
  } else {
    c.push(p('Ponto Um: [PONTO]', { bold: true, before: 200 }));
  }

  c.push(p('[CONTEÚDO DAS DELIBERAÇÕES]', { before: 200 }));
  ...
  (soc.socios || []).forEach(s => {
    ...
    c.push(p(s.nome || s.firma_socio || `[NOME_SOCIO_${s.letra}]`));
  });
...
export async function generateListaSocios(soc, opts = {}) {
  ...
  c.push(p(`${soc.firma}, ${soc.tipo?.toLowerCase()}, com o número de pessoa coletiva ${soc.nipc}, com sede em ${soc.sede}, com o capital social de ${fmtEur(soc.capital)}, representado pela${(soc.socios?.length || 0) > 1 ? 's seguintes quotas' : ' seguinte quota'}:`));
  (soc.socios || []).forEach((s, i) => {
    const libStr = s.quota_liberada ? ' — quota totalmente liberada' : '';
    c.push(p(`${String.fromCharCode(97 + i)}) Uma quota com o valor nominal de ${fmtEur(s.quota)}, representativa de ${s.pct}% do capital social, pertencente a ${idSocio(s)}${libStr}.`, { indent: true }));
  });
...
export async function generateContratoCessao(soc, opts = {}) {
  ...
  c.push(p(`"${soc.firma}", ${soc.tipo?.toLowerCase()}, com sede em ${soc.sede}, NIPC ${soc.nipc}, com o capital social de ${fmtEur(soc.capital)}.`));
  ...
  c.push(p(`O Cedente cede ao Cessionário, que aceita, a sua quota no valor nominal de ${cedente ? fmtEur(cedente.quota) : '[VALOR]'}, correspondente a ${cedente?.pct || '[%]'}% do capital social.`));
  c.push(p('CLÁUSULA SEGUNDA — Preço', { bold: true, center: true, before: 300 }));
  c.push(p(`A cessão é efetuada pelo preço global de ${fmtEur(opts.valor)}, pago pelo Cessionário ao Cedente na presente data.`));
  ...
  c.push(p('______________________________'));
  c.push(p(cedente?.nome || cedente?.firma_socio || '[NOME_CEDENTE]'));
  c.push(p('Pelo Cessionário,', { before: 300 }));
  c.push(p('______________________________'));
  c.push(p(cessionario?.nome || cessionario?.firma_socio || '[NOME_CESSIONÁRIO]'));
...
export async function generateAcordoParassocial(soc, opts = {}) {
  ...
  const cls = [
    ...
    ['CLÁUSULA SEXTA — Drag-Along', 'Caso uma Parte receba uma proposta para aquisição da totalidade do capital social aprovada pela maioria das Partes, as restantes ficam obrigadas a ceder as suas quotas nas mesmas condições.'],
    ...
    ['CLÁUSULA NONA — Não concorrência', '1. Durante a vigência do acordo, nenhuma Parte poderá exercer atividades concorrentes com o objeto social.\n2. Após saída, a obrigação mantém-se por 12 meses.'],
  ...
  (soc.socios || []).forEach((s, i) => {
    ...
    c.push(p('______________________________'));
    c.push(p(s.nome || s.firma_socio || `[NOME_${s.letra}]`));
  });
...
export async function generateDeclaracaoBemProprio(soc, opts = {}) {
  ...
  c.push(p(`O valor utilizado para a realização da participação social, com o valor nominal de ${fmtEur(s.quota)}, que ${s.nome} subscreveu na sociedade ${soc.firma}, NIPC ${soc.nipc}, é efetuado como bem próprio, não integrando o património comum do casal, nos termos da alínea c) do artigo 1723.º do Código Civil, dando ${s.conjuge_nome || '[CÔNJUGE]'} o seu expresso consentimento para o efeito.`, { before: 200 }));
